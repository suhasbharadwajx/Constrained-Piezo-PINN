...
print("MMS VALIDATION: NON-POLYNOMIAL & ASYMMETRIC WAVEFIELDS")
print(f"Using Compute Device: {device}")

model = PiezoInversePINN().to(device)

print("Pre-computing dynamic forcing terms (takes ~1-2 seconds)...")
N_colloc = 25000
...
X_sensor[:, 1] = 0.9 
u1_clean, u3_clean, phi_clean = manufactured_wavefield(X_sensor)

def add_awgn_tensor(signal_tensor, snr_db=30):
    sig_power = torch.mean(signal_tensor**2)
    if sig_power == 0: return signal_tensor
    noise_power = sig_power / (10 ** (snr_db / 10))
    noise = torch.randn_like(signal_tensor) * torch.sqrt(noise_power)
    return signal_tensor + noise

print("Injecting 30 dB Gaussian Noise into MMS Sensor Data...")
u1_ts = add_awgn_tensor(u1_clean, snr_db=30)
u3_ts = add_awgn_tensor(u3_clean, snr_db=30)
phi_ts = add_awgn_tensor(phi_clean, snr_db=30)
...
for ep in range(15000):
    ...
    if (ep + 1) % 1500 == 0:
        print(f"Epoch {ep+1:5d} | e15 = {model.e15_pred.item()*e33_SI:.5f} (True: {true_e15_SI:.5f}) | Loss: {loss.item():.2e}")
...
opt_lbfgs = torch.optim.LBFGS(
    model.parameters(), lr=0.1, max_iter=1500, history_size=100, 
...
t_plot = 0.5
pts = np.hstack([X_g.flatten()[:, None], Z_g.flatten()[:, None], np.full((res**2, 1), t_plot)])
...
ax1.set_title(rf"Robust MMS: Displacement $\bar{{u}}_3$ at t={t_plot}")
...
ax2.plot(z_l, u3_pred[:, slice_idx], 'b-', linewidth=2, label=r'Displacement $\bar{u}_3$')
ax2.plot(z_l, phi_pred[:, slice_idx], 'r--', linewidth=2, label=r'Electric Potential $\bar{\phi}$')
ax2.set_title(r"Asymmetric Coupled Response ($x_1 = 0.25$)")
...
ax4.axhline(y=true_e15_SI, color='red', linestyle='--', linewidth=2, label=f'True $e_{{15}}$ = {true_e15_SI}')
ax4.set_title("Robust Inverse Parameter Discovery")
ax4.set_xlabel("Optimization Steps")
ax4.set_ylabel(r"Dimensional $e_{15}$ (C/m$^2$)")
ax4.grid(True, alpha=0.3)
ax4.legend()

plt.tight_layout()
plt.savefig('PRA_Robust_MMS_Graphics.png', format='png', bbox_inches='tight')
plt.show()
print("Graphics successfully saved to 'PRA_Robust_MMS_Graphics.png'")
